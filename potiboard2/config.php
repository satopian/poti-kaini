<?php
/*
  * POTI-board改二 v2.20.6 lot.201212
  * by sakots >> https://poti-k.info/
  *
  * POTI-board改二の設定ファイルです。
  * setting file of POTI-board kaini
  *
*/
/* ---------- 最初に設定する項目 (set first) ---------- */
// 管理者パスワード
...
// By default setting, Rejected that name contain "通販".
...
// For example, Rejecting only "ブランド品のコピー" by default.
// Only "ブランド", or only "コピー" in the post, it is not rejected.
...
// set '0'. or not, set '1'.
// Original script is made by Japanese people to avoid spam from foreign countries, there is such a setting. excuse me.
...
// Settings of .htaccess file are working, you don't have to touch this. 
...
define('USE_NAME', '0');
define('DEF_NAME', '名無しさん');	//未入力時の名前(default name)

// 本文を必須にする する:1 しない:0
// To force They have any letter body, set '1', if not, set '0'.
define('USE_COM', '0');
define('DEF_COM', '本文無し');	//未入力時の本文(default letter body)

// 題名を必須にする する:1 しない:0
// To force They have title, set '1', if not, set '0'.
define('USE_SUB', '0');
define('DEF_SUB', '無題');	//未入力時の題名(default title)
...
// To DO NOT change the posting date and time even if they edit articles,
// set '1', if not, set '0'.
...
// Need the delete key to continue with newly posted pictures,
