...
@foreach ($ress as $res)
	...
	{{-- 親記事のヘッダ --}}
	<h3>
		<span class="name"><a href="search.php?page=1&amp;imgsearch=on&amp;query={{$res['encoded_name']}}&amp;radio=2" target="_blank" rel="noopener">{{$res['name']}}</a></span><span class="trip">{{$res['trip']}}</span> :
	...
	{{-- 子レスヘッダ --}}
		<h4>
			...
			<span class="name"><a href="search.php?page=1&amp;imgsearch=on&amp;query={{$res['encoded_name']}}&amp;radio=2" target="_blank" rel="noopener">{{$res['name']}}</a></span><span class="trip">{{$res['trip']}}</span> : {{$res['now']}}@if($res['id']) ID : {{$res['id']}}@endif @if($res['url']) <span class="url">[<a href="{{$res['url']}}" target="_blank" rel="nofollow noopener noreferrer">URL</a>]</span>@endif 
	...
	{{-- 親子共通 --}}
		...
		<div class="img_info_wrap">
			...
			@if($res['thumb']) - サムネイル表示中 - @endif @if($res['painttime']) PaintTime : {{$res['painttime']}}@endif
			<br>
			@if($res['continue']) <a href="{{$self}}?mode=continue&amp;no={{$res['continue']}}">●続きを描く</a>@endif @if($res['spch'])<span class="for_pc">@endif @if($res['pch']) <a href="{{$self}}?mode=openpch&amp;pch={{$res['pch']}}" target="_blank">●動画</a>@endif
