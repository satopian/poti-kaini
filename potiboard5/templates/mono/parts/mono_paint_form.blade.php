...
<form action="{{$self}}" method="post" enctype="multipart/form-data" id="paint_form">
	<p>
		...
		<br>
		@endif
		幅 : <input name="picw" type="number" title="幅" class="form" value="{{$pdefw}}" min="{{$pminw}}" max="{{$pmaxw}}" required>
		高さ : <input name="pich" type="number" title="高さ" class="form" value="{{$pdefh}}" min="{{$pminh}}" max="{{$pmaxh}}" required>
	@if($select_app)
		ツール :
