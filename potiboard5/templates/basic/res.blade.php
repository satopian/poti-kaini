...
<body>
	...
	<div id="body">
		<header>
			<h1 id="bbs_title">[{{$oya[0][0]['no']}}] {{$oya[0][0]['sub']}} @if($oya[0][0]['name'])<span
			...
			<div class="res_article_wrap">
				...
				<div class="article_info">
					<span class="article_info_name"><a
							href="{{$self}}?mode=search&page=1&imgsearch=on&query={{$res['encoded_name']}}&radio=2" target="_blank"
							rel="noopener">{{$res['name']}}</a></span>@if($res['trip'])<span
						class="article_info_trip">{{$res['trip']}}</span>@endif
					...
					@if($res['painttime'])<span class="article_info">描画時間:{{$res['painttime']}}</span>@endif
					...
					<div class="article_img_info">
						...
						@if($res['spch'])<span class="for_pc">@endif @if($res['pch'])@if($res['continue'])| @endif<span
								class="article_info_animation">☆<a
									...
									target="_blank" rel="opener">動画</a></span>@endif @if($res['spch'])</span>@endif
					</div>
					@endif
				</div>
				{{-- 記事共通ヘッダここまで --}}
				...
				// --> --}}
					...
					<tr>
						...
						<td><input type="file" name="upfile" accept="image/*">
							...
							<img id="attach_preview" style="max-width:200px;max-height:200px; display:none;" alt="preview" src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==">
						</td>
					</tr>
					@endif
					<tr>
