...
var DynamicColor=1,Palettes=[],ps=[];
// パレット配列作成
@if($palettes) 
...
{{$async}} function P_Effect(a){a=parseInt(a);let b,c=1;255==a&&(c=-1);const e=document.paintbbs;let f=String({{$await}} e.getColors()).split("\n");const d=f.length;let g="";for(b=0;d>b;b++){let h=a+parseInt("0x"+f[b].substring(1,3))*c,k=a+parseInt("0x"+f[b].substring(3,5))*c,l=a+parseInt("0x"+f[b].substring(5,7))*c;255<h?h=255:0>h&&(h=0);255<k?k=255:0>k&&(k=0);255<l?l=255:0>l&&(l=0);g+="#"+Hex(h)+Hex(k)+Hex(l)+"\n"}e.setColors(g);PaletteListSetColor()}
{{$async}} function PaletteMatrixGet(){const a=Palettes.length;var b=document.forms.Palette;const c=b.select;let e=b.setr;switch(b.m_m.selectedIndex){default:e.value="";let f=b=0;for(;a>b;)null!=c.options[b]&&(e.value=e.value+"\n!"+c.options[b].text+"\n"+Palettes[b],f++),b++;alert("\u30d1\u30ec\u30c3\u30c8\u6570\uff1a"+f+"\n\u30d1\u30ec\u30c3\u30c8\u30de\u30c8\u30ea\u30af\u30b9\u3092\u53d6\u5f97\u3057\u307e\u3057\u305f");break;case 1:e.value="!Palette\n"+String({{$await}} document.paintbbs.getColors()),alert("\u73fe\u5728\u4f7f\u7528\u3055\u308c\u3066\u3044\u308b\u30d1\u30ec\u30c3\u30c8\u60c5\u5831\u3092\u53d6\u5f97\u3057\u307e\u3057\u305f")}e.value=
e.value.trim()+"\n!Matrix"}
...
function PaletteSet(){var a=document.forms.Palette;const b=a.setr.value,c=a.select;var e=a.m_m.selectedIndex;a=b.length;let f;if(1>a)alert("\u30de\u30c8\u30ea\u30af\u30b9\u60c5\u5831\u304c\u3042\u308a\u307e\u305b\u3093\u3002");else{var d;switch(e){default:for(d=c.length;0<d;)d--,c.options[d]=null;case 2:e=c.options.length;d=b.indexOf("!",0)+1;if(0==d)return;for(;d<a;){var g=b.indexOf("\n#",d);if(-1==g)return;const h=b.substring(d,g+1);d=b.indexOf("!",g);if(-1==d)return;f=b.substring(g+1,d+-1);"Palette"!=
h?(0<=e&&(c.options[e]=new Option(h)),Palettes[e]=f,e++):document.paintbbs.setColors(f);d+=1}break;case 1:d=b.indexOf("!",0)+1;if(0==d)return;g=b.indexOf("\n#",d);d=b.indexOf("!",g);0<=g&&(f=b.substring(g+1,d-1));document.paintbbs.setColors(f)}PaletteListSetColor()}}function PaletteListSetColor(){let a;const b=document.forms.Palette.select;for(a=1;b.options.length>a;a++){var c=Palettes[a].split("\n");b.options[a].style.background=c[4];b.options[a].style.color=GetBright(c[4])}}
function GetBright(a){let b=parseInt("0x"+a.substring(1,3)),c=parseInt("0x"+a.substring(3,5));a=parseInt("0x"+a.substring(5,7));a=b>=c?b>=a?b:a:c>=a?c:a;return 128>a?"#FFFFFF":"#000000"}function Chenge_(){var a=document.forms.grad,b=a.pst.value;a=a.ped.value;isNaN(parseInt("0x"+b))||isNaN(parseInt("0x"+a))||GradView("#"+b,"#"+a)}
function ChengeGrad(){var a=document.forms.grad,b=a.pst.value,c=a.ped.value;Chenge_();var e=parseInt("0x"+b.substring(0,2)),f=parseInt("0x"+b.substring(2,4)),d=parseInt("0x"+b.substring(4,6));b=Math.trunc((e-parseInt("0x"+c.substring(0,2)))/15);a=Math.trunc((f-parseInt("0x"+c.substring(2,4)))/15);c=Math.trunc((d-parseInt("0x"+c.substring(4,6)))/15);isNaN(b)&&(b=1);isNaN(a)&&(a=1);isNaN(c)&&(c=1);var g="";let h;for(h=0;14>h;h++,e-=b,f-=a,d-=c){if(255<e||0>e)b*=-1,e-=b;if(255<f||0>f)a*=-1,f-=a;if(255<
d||0>d)c*=-1,f-=c;g+="#"+Hex(e)+Hex(f)+Hex(d)+"\n"}document.paintbbs.setColors(g)}function Hex(a){a=Math.trunc(a);0>a&&(a*=-1);for(var b=new String,c;16<a;)c=a,16<a&&(a=Math.trunc(a/16),c-=16*a),c=Hex_(c),b=c+b;c=Hex_(a);for(b=c+b;2>b.length;)b="0"+b;return b}function Hex_(a){isNaN(a)?a="":10==a?a="A":11==a?a="B":12==a?a="C":13==a?a="D":14==a?a="E":15==a&&(a="F");return a}
{{$async}} function GetPalette(){var a=String({{$await}} document.paintbbs.getColors());if("null"!=a&&""!=a){ps=a.split("\n");a=document.forms.grad;var b=a.p_st.selectedIndex,c=a.p_ed.selectedIndex;a.pst.value=ps[b].substring(1,7);a.ped.value=ps[c].substring(1,7);GradSelC();GradView(ps[b],ps[c]);PaletteListSetColor()}}
function GradSelC(){const a=document.forms.grad;let b;if(a.view.checked){var c=ps.length,e="";for(b=0;c>b;b++){let f=255+-1*parseInt("0x"+ps[b].substring(1,3)),d=255+-1*parseInt("0x"+ps[b].substring(3,5)),g=255+-1*parseInt("0x"+ps[b].substring(5,7));255<f?f=255:0>f&&(f=0);255<d?d=255:0>d&&(d=0);255<g?g=255:0>g&&(g=0);e+="#"+Hex(f)+Hex(d)+Hex(g)+"\n"}e=e.split("\n");for(b=0;c>b;b++)a.p_st.options[b].style.background=ps[b],a.p_st.options[b].style.color=e[b],a.p_ed.options[b].style.background=ps[b],
a.p_ed.options[b].style.color=e[b]}}function GradView(a,b){}function showHideLayer(){const a=document.forms.grad;var b;(b=(b=document.getElementById("psft"))?b.style:null)&&!a.view.checked&&(b.visibility="hidden");b&&a.view.checked&&(b.visibility="visible",GetPalette())};
	</script>
	<!--動的パレットスクリプト ここまで-->
	<noscript>
	...
	<div class="paintbbs_memo">
		...
		<div>
			<ul>
				...
				<li>カラーパレット郡<br>クリックで色取得。右クリックで色の登録。Shift+クリックでデフォルト値。<br><br></li>
