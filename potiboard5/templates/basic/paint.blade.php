...
function GetBright(a){let b=Number("0x"+a.substring(1,3)),c=Number("0x"+a.substring(3,5));a=Number("0x"+a.substring(5,7));a=b>=c?b>=a?b:a:c>=a?c:a;return 128>a?"#FFFFFF":"#000000"}function Chenge_(){var a=document.forms.grad,b=a.pst.value;a=a.ped.value;isNaN(Number("0x"+b))||isNaN(Number("0x"+a))||GradView("#"+b,"#"+a)}
function ChengeGrad(){var a=document.forms.grad,b=a.pst.value,c=a.ped.value;Chenge_();var e=Number("0x"+b.substring(0,2)),f=Number("0x"+b.substring(2,4)),d=Number("0x"+b.substring(4,6));b=Math.trunc((e-Number("0x"+c.substring(0,2)))/15);a=Math.trunc((f-Number("0x"+c.substring(2,4)))/15);c=Math.trunc((d-Number("0x"+c.substring(4,6)))/15);isNaN(b)&&(b=1);isNaN(a)&&(a=1);isNaN(c)&&(c=1);var g="";let h;for(h=0;14>h;h++,e-=b,f-=a,d-=c){if(255<e||0>e)b*=-1,e-=b;if(255<f||0>f)a*=-1,f-=a;if(255<d||0>d)c*=
-1,f-=c;g+="#"+Hex(e)+Hex(f)+Hex(d)+"\n"}document.paintbbs.setColors(g)}function Hex(a){a=Math.trunc(a);0>a&&(a*=-1);for(var b=new String,c;16<a;)c=a,16<a&&(a=Math.trunc(a/16),c-=16*a),c=Hex_(c),b=c+b;c=Hex_(a);for(b=c+b;2>b.length;)b="0"+b;return b}function Hex_(a){isNaN(a)?a="":10==a?a="A":11==a?a="B":12==a?a="C":13==a?a="D":14==a?a="E":15==a&&(a="F");return a}
