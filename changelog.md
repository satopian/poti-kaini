...
HTML内のJavascriptの記述の変更があるため、このテンプレートの更新は必須になります。  
変更後は今回更新したlitaChixで動作するようになり、また以前のバージョンのlitaChixでも動作します。    
...
- サーバの設定やユーザのセキュリティソフトや広告ブロックによってv6.133.0で追加したlitaChixのウェブワーカーがブロックされてしまい、投稿できなくなるケースがある問題を修正しました。  
...
### 2026/02/12 v6.136.0
### litaChix更新
- キャンバスの回転、手のひらツール使用時にのみ有効なピンチズームのためのTouchEventが、描画時にも有効になっていたのを修正しまいた。  
- TouchEventによって描画時に遅延が発生していたのを修正しました。

###  絵文字が拒絶されていたのを修正
- v6.129.1で追加したZalgo text（ザルゴ・テキスト）を拒絶する正規表現によって、正当な絵文字まで拒絶されていたのを修正しました。  
...
### 2026/01/27 v6.128.3
### AXNOS Paint更新
- さとぴあ改造版のAXNOS Paintがスマホで正常に動作しなくなっていたのを修正しました。
...
### 型の統一
- true/falseのboolとarrayが混在していた変数をarrayに統一しました。
...
### 2025/12/07 v6.115.1
### ｢litaChix｣の大きなバグを修正
- v6.115.0でズームアウト時のスムージングの品質を｢高｣に設定しましたが、それが原因でlitaChixの描画速度が極端に落ちてしまい、描画遅延が発生していました。  
...
<img width="929" height="810" alt="image" src="https://github.com/user-attachments/assets/defdafa2-d850-414e-9091-30eba16d6936" />
...
<img width="212" height="261" alt="image" src="https://github.com/user-attachments/assets/726d2f0e-07a0-4883-bd83-930d182ffa89" /><br>
...
### 2025/06/23  v6.80.8
- 指定日数を超過した古い記事の｢続きを描く｣リンクがでない仕様ですが、リンクが存在しなくても、urlを直打ちすると｢続きを描く｣画面が表示される事が判明しました。  
続きを描く処理に指定日数を超過しているかどうかを確認する処理を追加して、指定日数を超過している時はエラーメッセージを表示するようにしました。 
...
｢◆CSZ6G0yP9Q｣という文字しかわからない場合は、もとの入力がなんだったのかを推測する事が難しいため、匿名掲示板で本人である事の証明する時に使われているものです。   
