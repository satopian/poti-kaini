...
日付が変わってもipアドレスが同じ時は同じIDが表示されるようになりました。  
1日の投稿件数が数十件に及ぶケースでは一日限りのIDでも使いみちがありますが、数日間に数件しか投稿がない掲示板ではIDが意味をなしません。  
そのため、config.phpで設定したIDの種とipアドレスのみでIdを生成する形に変更されました。  
