...
PHPのバージョンがPHP5.3以下の時にもPHPのバージョンが低い事が原因で動作しない事を表示するようになりました。  
これまでは、致命的エラーになっていました。そのため設置成功しない原因がわからなくなっていました。  
...
その場合は各作者の方に独自に対応してもらうか、自分でコードを追加するなどの対応をお願いします。
```
<script>
	function fixchicken() {
...
iPad以上の画面の横幅を検出した時にはNEOのキャンバスの周囲の網目の上でスワイプしても画面が動かないようにしました。  
...
- picpost.php
エラーメッセージの日本語･英語自動切り替えがiPadのja-jpに対応していなかったを修正しました。  
これまでは、iPadまたはja-jpを返す端末でエラーが発生した時に英語のエラーメッセージが表示されていました。
