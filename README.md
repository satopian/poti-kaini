...
- 脆弱なパスワードが使用されるのを防ぐためパスワードが5文字以下の時はエラーメッセージを出すようにしました。｢パスワードが短すぎます。最低6文字。｣
- 設定した日数より古いスレッドへの返信をロックする機能を拡張して編集もできなくなるようにしました。  
...
このデフォルト値を｢する:1｣ から ｢しない:0｣ に変更しました。
これまでは新しいテーマのHTMLファイルを使っていてもconfig.phpのバージョンが古い時は｢画像なし｣のチェックを入れたり外したりする必要がありました。  
